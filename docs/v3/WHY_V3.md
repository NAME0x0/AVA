...
3. **PrismML Bonsai (Caltech, March–April 2026).** First commercially-viable 1-bit LLM family. **1.125 bpw** packed storage (1-bit weight ±1, 128 weights share one FP16 scale). Bonsai 8 B fits in **1.15 GB**. Ternary Bonsai (1.58-bit) follow-on family in 1.7 B / 4 B / 8 B sizes. Kernels in the `PrismML-Eng/llama.cpp` fork and Apple's MLX; v3 takes the *evidence* (sub-2-bpw works at 8 B) and ships **stock upstream TQ1_0/TQ2_0** instead — see [PRISMML.md](PRISMML.md).
