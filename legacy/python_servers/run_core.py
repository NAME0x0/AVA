...
def display_stats(ava_system) -> None:
    ...
    agency_table.add_row(
        "Avg Free Energy", f"{agency_stats.get('avg_expected_free_energy', 0):.3f}"
    )
...
async def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="AVA v3 Core System - Cortex-Medulla Architecture")
    parser.add_argument(
        "--config",
        "-c",
        type=str,
