"""Fast QLoRA fine-tuning using Unsloth.

Unsloth provides 2-5x training speedup and ~30% VRAM reduction
compared to standard HuggingFace Trainer.
"""
...
class UnslothFinetuneConfig:
    ...
    corpus_path: str = "D:/AVA/experiments/exp4_finetune/corpora/ava_exp4_finetune_fast.jsonl"
...
def format_for_training(examples: list[dict], tokenizer) -> list[str]:
    """Format examples as chat-templated strings for SFTTrainer."""
    formatted = []
    for ex in examples:
...
def run_unsloth_finetune(config: UnslothFinetuneConfig | None = None) -> dict:
    config = config or UnslothFinetuneConfig()

    print("=" * 60)
    print("AVA Experiment 4: Unsloth QLoRA Fine-tuning")
    print("=" * 60)
    ...
    # Load with standard bitsandbytes 4-bit (proven to work on 4GB VRAM)
    print("\nLoading tokenizer...")
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        attn_implementation="sdpa",  # Use PyTorch's Flash/Efficient SDP
    )
    ...
    # Re-tokenize as input_ids for standard Trainer
    ...
    training_args = TrainingArguments(
        ...
        eval_strategy="no",  # Eval OOMs due to logits.float() on 248K vocab
        seed=config.seed,
    )
    ...
    # Skip eval (OOMs due to logits.float() on 248K vocab without gradient checkpointing)
    # Will eval externally after training via benchmark scripts
