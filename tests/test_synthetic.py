...
def test_generate_tool_repair_examples_targets_direct_trace_and_refusal_modes() -> None:
    ...
    assert any(
        item["response"] == "I cannot help with that."
...
def test_generate_public_reasoning_patch_examples_balances_labels_and_preserves_tool_anchors() -> (
    None
):
    ...
    choices = [
        item["response"] for item in examples if item["kind"] in {"science_mc", "commonsense_mc"}
    ]
...
def test_generate_public_benchmark_distill_examples_uses_train_splits_and_anchors() -> None:
    ...
    examples = generate_public_benchmark_distill_examples(
        arc_limit=3, gsm8k_limit=4, arc_rotations=1, anchor_repeats=2
    )
        ...
        [
            {
                ...
                "choices": {
                    "label": ["A", "B", "C", "D"],
                    "text": ["magnetism", "gravity", "friction", "sound"],
                },
                "answerKey": "B",
            }
        ],
