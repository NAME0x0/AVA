...
def _build_sparse_support_index(examples: list[SupportExample]) -> SparseSupportIndex:
    ...
    for example in examples:
        ...
        option_tokens = tuple(
            _support_lookup_tokens(" ".join(choice_text for _label, choice_text in choices))
        )
...
def _sparse_support_score(
    query_counts: Counter[str], support_counts: dict[str, int], idf: dict[str, float]
) -> float:
    shared = set(query_counts) & set(support_counts)
    if not shared:
        return 0.0
    numerator = sum(
        min(query_counts[token], support_counts[token]) * idf.get(token, 1.0) for token in shared
    )
    denominator = sum(query_counts[token] * idf.get(token, 1.0) for token in query_counts)
    return numerator / max(denominator, 1e-9)


def _support_response_choice_score(
    response_tokens: frozenset[str], choice_tokens: frozenset[str]
) -> float:
...
def _support_graph_edge_score(
    left_counts: dict[str, int], right_counts: dict[str, int], idf: dict[str, float]
) -> float:
...
def _propagate_support_scores(
    ...
    *,
    ...
    scored_indices = sorted(
        row_indices, key=lambda index: base_scores.get(index, 0.0), reverse=True
    )
    ...
    for candidate_index in row_indices:
        ...
        for seed_index in seeds:
            edge = _support_graph_edge_score(
                row.question_counts,
                support_index.rows[seed_index].question_counts,
                support_index.idf,
            )
            if edge <= 0.0:
                continue
...
def _top_support_details(
    ...
    *,
    ...
    for index in ranked[:top_k]:
        details.append(
            {
                "score": round(float(scores.get(index, 0.0)), 6),
                **support_index.rows[index].reference,
            }
        )
    return details
...
def _predict_multiple_choice_from_support(
    *,
    ...
    scored = sorted(
        (
            (
                _sparse_support_score(
                    query_counts, support_index.rows[index].prompt_counts, support_index.idf
                ),
                index,
            )
    ...
    choice_token_sets = {
        label: frozenset(_support_lookup_tokens(choice_text)) for label, choice_text in choices
    }
    for score, index in scored:
        ...
        for label, _choice_text in choices:
            choice_scores[label] += score * _support_response_choice_score(
                row.response_token_set, choice_token_sets[label]
            )
...
def _predict_multiple_choice_hybrid_from_support(
    *,
    ...
    for label, choice_text in choices:
        ...
        for index in candidate_indices:
            row = support_index.rows[index]
            question_match = _sparse_support_score(
                question_counts, row.question_counts, support_index.idf
            )
            prompt_match = base_scores.get(index, 0.0)
            option_match = (
                _sparse_support_score(option_counts, row.option_counts, support_index.idf)
                if option_counts
                else 0.0
            )
            full_match = _sparse_support_score(
                choice_query_counts, row.full_counts, support_index.idf
            )
            response_match = _support_response_choice_score(
                row.response_token_set, choice_token_set
            )
            semantic_match = (
                1.0 if _normalize_text(row.response) == _normalize_text(choice_text) else 0.0
            )
            total = (
                (0.35 * question_match)
                + (0.05 * prompt_match)
    ...
    best_label = max(choice_scores.items(), key=lambda item: item[1])[0]
    combined_scores = {
        label: round(float(choice_scores[label]), 6) for label, _choice_text in choices
    }
    return best_label, {
        "mode": "hybrid_support_mc",
        "scores": combined_scores,
        "selected": best_label,
        "supports": _top_support_details(
            support_index, propagated_scores, candidate_indices, top_k=top_k
        ),
...
def _predict_model_retrieval_ensemble(
    ...
    *,
    ...
    return best_label, {
        ...
        "model_scores": model_scores,
        "retrieval_scores": {
            label: round(float(retrieval_scores.get(label, 0.0)), 6) for label, _ in choices
        },
        "model_probs": {choices[i][0]: round(model_probs[i], 6) for i in range(len(choices))},
        "retrieval_probs": {
            choices[i][0]: round(retrieval_probs[i], 6) for i in range(len(choices))
        },
    ...
    *,
    ...
    sparse_prediction = (
        ...
        if support_resource is not None and support_resource.get("kind") == "banks"
            ...
            choices=choices,
            support_examples=(
                list(support_resource.get("examples", [])) if support_resource else None
            ),
    ...
    dense_top_score = (
        _rerank_top_score(dense_scoring)
        if support_reranker is not None
        else _dense_shortlist_top_score(dense_scoring)
    )
    ...
    *,
    ...
    for bank_name in bank_order:
        ...
        margin = _score_margin(
            {str(key): float(value) for key, value in dict(scoring.get("scores", {})).items()}
        )
...
def _load_cached_rows(
    parts: tuple[str, ...], filename: str, limit: int | None, offset: int = 0
...
def load_gsm8k_tasks(
    *, split: str = "test", limit: int | None = 50, offset: int = 0
...
def load_arc_challenge_tasks(
    *, split: str = "validation", limit: int | None = 50, offset: int = 0
) -> list[ExternalBenchmarkTask]:
    rows = _load_cached_rows(
        ("allenai___ai2_arc", "ARC-Challenge", "0.0.0"), f"ai2_arc-{split}.arrow", limit, offset
    )
    if rows is None:
        if not DATASETS_AVAILABLE:
            raise RuntimeError("datasets is required for external benchmarks.")
        rows = list(
            load_dataset(
                "allenai/ai2_arc", "ARC-Challenge", split=_slice_split(split, limit, offset)
            )
        )
    tasks: list[ExternalBenchmarkTask] = []
    for row in rows:
...
def evaluate_external_benchmark(
    ...
    *,
    ...
    if support_resource is None:
        ...
        primary_bank = str(
            dict(support_resource["manifest"]).get("primary_bank")
            or next(iter(dict(support_resource["banks"])))
        )
    ...
    dense_support_resource = (
        _load_support_resource(dense_support_corpus) if dense_support_corpus else support_resource
    )
    ...
    return {
        ...
        "nearest_threshold": nearest_threshold,
        "nearest_margin": nearest_margin,
        "dense_encoder_model": dense_encoder_model
        if retrieval_mode
        in {
            "dense_hybrid_support_mc",
            "dense_rerank_support_mc",
        ...
        "dense_encoder_device": dense_encoder_device
        if retrieval_mode
        in {
            "dense_hybrid_support_mc",
            "dense_rerank_support_mc",
        ...
        "dense_candidate_top_k": dense_candidate_top_k
        if retrieval_mode
        in {
            "dense_hybrid_support_mc",
            "dense_rerank_support_mc",
        ...
        "router_dense_score_min": router_dense_score_min
        if retrieval_mode
        in {"sparse_dense_router_support_mc", "sparse_dense_rerank_router_support_mc"}
        else None,
        "router_dense_margin_min": router_dense_margin_min
        if retrieval_mode
        in {"sparse_dense_router_support_mc", "sparse_dense_rerank_router_support_mc"}
        else None,
        "router_sparse_margin_max": router_sparse_margin_max
        if retrieval_mode
        in {"sparse_dense_router_support_mc", "sparse_dense_rerank_router_support_mc"}
        else None,
        "router_margin_gap_min": router_margin_gap_min
        if retrieval_mode
        in {"sparse_dense_router_support_mc", "sparse_dense_rerank_router_support_mc"}
        else None,
